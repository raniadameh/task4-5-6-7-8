...
export class CreateUpdatePostComponent implements OnInit {
  ...
  constructor(private location: Location, private postsService: UserPostsService, private router: Router, private route: ActivatedRoute) { }
    ...
    if (this.postId == '') {
      ...
      this.postsService.getmyPost(this.postId).subscribe(response => {
        ...
        this.postForm = new FormGroup({
          ...
          likes: new FormControl(this.viewPost.likes),
          tages: new FormControl(this.viewPost.tags),
          image: new FormControl(this.viewPost.image),
          owner: new FormControl(this.ownerId, [Validators.required])
        });
        this.postForm.controls['owner'].disable();
      }
  ...
  get tages() {
    return this.postForm.get('tages');
  }
  get image() {
